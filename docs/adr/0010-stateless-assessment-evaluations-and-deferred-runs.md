...
2. The `allergen_assessment` contract is always returned on every candidate, reporting explicit outcomes (`NOT_ASSESSED` with `FEATURE_DISABLED` while disabled, or evidence-derived outcomes once enabled).
