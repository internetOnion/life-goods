...
The production translation configuration is `v1`, with the expanded field selection and exact provider/model identity in its fingerprint. It excludes incompatible configurations and `test-fake` artifacts without deleting them. Complete results are durable; partial results use only the short-lived hot cache. Missing credentials disable generation while allowing compatible stored results to be read.
