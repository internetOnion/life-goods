...
<body>
  ...
  <script>
      ...
      $("panels").innerHTML = state.products.map((product) => `
        <article class="panel" aria-labelledby="${product.id}-title">
          ...
          <div class="panel-body">
            <div class="dropzone"><strong>Add label photos</strong><span>Start with the Nutrition Facts panel. Optional photos can show the Product front, package quantity, or a wrapped or additional nutrition panel.</span><input class="file-input" data-add="${product.id}" type="file" accept="image/jpeg,image/png,image/heic,image/heif,.heic,.heif" multiple></div>
            <div class="previews" data-previews="${product.id}">${product.photos.map((photo, index) => `<figure class="preview" data-preview="${escapeHtml(photo.localId)}"><img src="${photo.url}" alt="${escapeHtml(product.title)} photo ${index + 1}"><span class="preview-label">Photo ${index + 1}</span><span class="preview-controls"><button class="icon-button" data-replace="${product.id}:${index}" type="button" title="Replace photo ${index + 1}" aria-label="Replace photo ${index + 1}">↻</button><button class="icon-button" data-remove="${product.id}:${index}" type="button" title="Remove photo ${index + 1}" aria-label="Remove photo ${index + 1}">×</button></span><input hidden data-replace-input="${product.id}:${index}" type="file" accept="image/jpeg,image/png,image/heic,image/heif,.heic,.heif"></figure>`).join("")}</div>
    ...
    function replacePhoto(product, index, file) { if (!file || !["image/jpeg", "image/png", "image/heic", "image/heif", ""].includes(file.type)) return; const old = product.photos[index]; if (old) URL.revokeObjectURL(old.url); product.photos[index] = { file, url: URL.createObjectURL(file), localId: crypto.randomUUID() }; invalidateProduct(product); render(); }
