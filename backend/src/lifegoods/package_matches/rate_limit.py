...
class RedisPackageMatchRateLimiter:
    ...
    def _record_degradation(self) -> None:
        ...
        if should_log:
            logger.warning(
                "Package Match rate limiter entered local fallback",
                extra={
                    "event": "package_match_rate_limit_degraded",
                    "dependency": "redis",
    ...
    def _record_recovery(self) -> None:
        ...
        if should_log:
            logger.info(
                "Package Match rate limiter recovered shared limiting",
                extra={
                    "event": "package_match_rate_limit_recovered",
                    "dependency": "redis",
                },
            )
    ...
    def _try_redis(self, digest: str) -> tuple[bool, int]:
        ...
        raise RedisError("Package Match rate-limit transaction contention")
