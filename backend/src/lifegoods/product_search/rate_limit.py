...
class ProductSearchRateLimiter(Protocol):
    def try_acquire(self, key: str) -> tuple[bool, int]: ...


class RedisProductSearchRateLimiter(RedisSlidingWindowRateLimiter):
    def __init__(
        self,
        client: redis.Redis,
        ...
        *,
        ...
        super().__init__(
            client,
            requests_per_minute,
            key_prefix="product-search:rate-limit",
            display_name="Product Search",
            event_prefix="product_search",
            logger=logger,
