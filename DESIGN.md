...
One centered reading rail: maximum `36rem`, `1rem` mobile gutters widening to `1.5rem` at `sm`. Minimum viewport width is 320px; every layout must survive it. The sticky header is `4rem` high, and the Product section rail pins immediately beneath it. Product sections stack inside one white sheet separated by hairlines with `2.5rem` vertical padding. Detail rows use a `10rem` label column only when space supports it; nutrition reads as a labelled stacked list on phones and a right-aligned table from `sm`, with columns derived from supplied data only. Space above a heading is always larger than the space below it. Compare Products uses a two-step Product A → Product B capture control with `0.75rem` top and bottom separation on mobile, widening to `1rem` at `sm`; the current step carries amber emphasis and each step remains a 44px touch target. Comparison results render on their own page, led by Product identities and the comparison basis, with a clear return to edit either Product. Read This Label opens on an always-visible capture board: while empty, an unfolded carton marked 1 (front), 2 (back), and a dashed 3 (optional side) sits above the three step rows, and every row carries its own Take photo and Choose from library actions, so a photo only ever lands on the step that was tapped; a photo on the wrong step is removed and added again on the right one. Start photos opens a camera-first path: the dark camera sheet carries a top rail of three 44px step wells joined by a line that turns amber as each photo settles into its well, a per-step faint outline of what that package face usually carries inside the frame, and a round amber shutter. After the camera closes, the same board shows thumbnails, and the next required step is marked up next. While photos are read, both Nutrition Labels modes show one shared waiting panel on a white sheet: the Shopper's own photos as 4rem thumbnails with an amber scan line over those being read (checked and dimmed when done, faded when pending), a mono elapsed timer, time-honest copy that turns to a warning-tone still-working note after 45 seconds, observable stages only (Compare: A, B, compare), and an outline Cancel; the capture dock hides while it shows.
