            ...
            <CardContent className="space-y-4 p-4 pt-2 text-xs sm:p-5 sm:pt-2">
                ...
                {percent !== null && (
                    <div className="space-y-1.5">
                        <div className="flex items-center justify-between text-xs sm:text-sm">
                            ...
                            <span className="font-mono text-xs font-bold text-neutral-950 tabular-nums sm:text-sm">
                                {percent}%
                            </span>
                        </div>
                ...
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                    {completedStates.length > 0 && (
                        <div className="space-y-1.5">
                            <span className="flex items-center gap-1 text-xs font-bold text-neutral-900 sm:text-sm">
                            ...
                            <div className="flex flex-wrap gap-1">
                                {completedStates.map((s, idx) => (
                                    <span
                                        key={idx}
                                        className="rounded-md border border-emerald-200/60 bg-emerald-50 px-2 py-0.5 text-xs font-semibold text-emerald-800 capitalize"
                                    >
                                        {s}
                                    </span>
                                ))}
                        ...
                        <div className="space-y-1.5">
                            ...
                            <div className="flex flex-wrap gap-1">
                                {toCheckStates.map((s, idx) => (
                                    <span
                                        key={idx}
                                        className="rounded-md border border-neutral-200 bg-neutral-100 px-2 py-0.5 text-xs font-semibold text-neutral-700 capitalize"
                                    >
                                        {s}
                                    </span>
                                ))}
                ...
                <div className="flex flex-wrap items-center justify-between gap-2 border-t border-neutral-100 pt-2 text-xs font-medium text-neutral-500">
                    {creator && (
