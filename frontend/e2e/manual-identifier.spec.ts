...
test('keyboard and live regions expose invalid, loading, failure, and retry states', async ({ page }) => {
  let attempts = 0
  ...
  await expect(page.getByRole('heading', { name: 'មិនអាចពិនិត្យបានឥឡូវនេះ' })).toBeVisible()
  await expect(page.getByRole('status')).toContainText('មិនអាចពិនិត្យបានឥឡូវនេះ')

  await expect(page.getByRole('button', { name: 'ព្យាយាមម្ដងទៀត' })).toBeFocused()
  await page.keyboard.press('Enter')
...
test('real Khmer copy wraps without clipping at representative narrow widths', async ({ page }) => {
  for (const viewport of [
    ...
    await page.goto('/')
    const clippedElements = await page.locator('h1, p, label, button').evaluateAll((elements) =>
      elements.filter(
        (element) =>
